...
- `mapi<<+;3>>(A)` 会把 0 基下标作为第一个操作数，降级为类似 `[0 + A[0], 1 + A[1], ...]` 的数组表达式。
- `foldi<<+;3>>(init,A)` 会先构造 `(i + A[i])`，再把这些下标感知项左结合折叠进累加器。
- `mapfold<<+;3>>(init,A,B)` 是首版“先 map、再 fold 同一批映射值”的元组返回语义，可用于 `m, s = mapfold...` 这类多左值赋值。
- 计数 `n` 必须是正整数字面量；操作符目前限内置二元操作符及对应 `$...$` 别名。
- `mapw`、`mapwi`、`foldw`、`foldwi` 以及用户自定义迭代器操作符会显式报不支持错误。
